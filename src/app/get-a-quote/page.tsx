import Image from "next/image"
import Link from "next/link"


import model from "./images/model.png"


export default function Get_A_Qoute() {

  return (
    <>
      <main className="bg-[#eee] py-5 md:py-15 px-3">
        <div className="p-5 md:p-10 max-w-4xl mx-auto bg-white rounded-3xl">
          
          <h1 className="md:w-2/3 text-xl md:text-3xl text-black font-bold text-balance text-center mb-8 mx-auto">
            Get a fast, personalized quote for your vehicle or Fleets wrap.
          </h1>

          <form className="flex flex-col gap-4">

            <h3 className="font-semibold text-xl">Select Your Vehicle</h3>

            <div className="flex flex-col md:flex-row gap-4"> {/* first input div */}
              <div className="md:w-1/2 flex flex-col gap-2">
                <label htmlFor="make" className="font-semibold">Make</label>
                <input
                  type="text"
                  className="placeholder:text-black border border-t-0 border-r-0 border-l-0 border-b-1 p-1"
                  placeholder="Ford"
                />
              </div>
              <div className="md:w-1/2 flex flex-col gap-2">
                <label htmlFor="model" className="font-semibold">Model</label>
                <input
                  type="text"
                  className="placeholder:text-black border border-t-0 border-r-0 border-l-0 border-b-1 p-1"
                  placeholder="E-Transit"
                />
              </div>
            </div>

            <div className="flex flex-col md:flex-row gap-4"> {/* second input div */}
              <div className="md:w-1/2 flex flex-col gap-2">
                <label htmlFor="year" className="font-semibold">Year</label>
                <input
                  type="text"
                  className="placeholder:text-black border border-t-0 border-r-0 border-l-0 border-b-1 p-1"
                  placeholder="2025"
                />
              </div>
              <div className="md:w-1/2 flex flex-col gap-2">
                <label htmlFor="number" className="font-semibold">Your VIN Number</label>
                <input
                  type="text"
                  className="placeholder:text-black border border-t-0 border-r-0 border-l-0 border-b-1 p-1"
                  placeholder="2HGFB2F55DH017656"
                />
              </div>
            </div>

          </form>

          <Image
            className="w-full h-full my-8"
            src={model}
            alt="model"
          />

          <button className="w-full py-3 bg-green-600 text-white font-semibold"><Link href="/get-a-quote/quote-2">Next</Link></button>
        </div>
      </main>
    </>
  )
}